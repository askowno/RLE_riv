...
#overall summary
riv_type_14 <- riv %>%
    group_by(RIVTYPE, PES2014) %>%
  summarise(leng = sum(as.numeric(leng), na.rm = TRUE)) %>%
  ungroup() %>%
  pivot_wider(names_from = PES2014, values_from = leng, values_fill = list(leng = 0))  %>%
  mutate(tot_leng14 = rowSums(across(c(A, B, C, D, E, F, `Data defic`))))
...
# Join the tables above - this makes a single table in which all the required metric can be found to compute EPL and RLE
 ...
 riv_type_metrics <- riv_type_14 %>%
   rename(A14=A, B14=B, C14=C, D14=D, E14=E, F14=F, DDef14 = `Data defic`) %>%
   left_join(riv_type_18, by = "RIVTYPE") %>%
...
# add in change in PES for B1aiii ongoing decline evidence (PES ABC DD extent decline > 1%)  
# length of good river (ABC&DDef) declines by >1% from t1 to t2
   mutate(pes_decline1418 = ifelse(
      (((A14 + B14 + C14 + DDef14) - (A18 + B18 + C18 + DDef18))
      / (A14 + B14 + C14 + DDef14)) 
      > 0.01, "decline1418", "nodecline1418"))  %>%
    mutate(pes_decline1824 = ifelse(
      (((A18 + B18 + C18 + DDef18) - (A + B + C + DDef))
      / (A18 + B18 + C18 + DDef18)) 
      > 0.01, "decline1824", "nodecline1824"))  %>%
# add D3 using PES as measure of functional decline due to biotic disruption (and environmental degradation) 
# PES EF ~ 90%-100% severity; DEF ~ 70%-100% severity; CDEF ~ 50%-100% severity
    ...
    mutate(D3_24 = case_when( prp24_ef >= 0.9 ~ "CR", 
                                 ...
                                 prp24_ef  >= 0.5 | prp24_def >=0.7 | 
                                                    prp24_cdef >= 0.9 ~ "VU", 
                                 TRUE ~ "LC")) %>%
# add B1aiii using declining PES trend as evidence of ongoing decline (calculated above in pes_decline)
   mutate(B1_18 = case_when( pes_decline1418 == "decline1418" & eoo_km2  <= 2000 ~ "CR",
                             pes_decline1418 == "decline1418" & eoo_km2  <= 20000 ~ "EN",
                             pes_decline1418 == "decline1418" & eoo_km2  <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
   ...
   mutate(RLE_24 = case_when(B1_24 == "CR" | D3_24 == "CR" ~ "CR", 
                             ...
                             B1_24 == "VU" | D3_24 == "VU" ~ "VU",
                              TRUE ~ "LC")) %>%
# add van Deventer 2018 method: SA approach assuming A and B are intact rest is lost but also counting C towards VU
...
Plot of Red List Index of Ecosystems for 2018 and 2014 using RLE and SA methods

```{r}
## RLIE plot 2018-2014 using SA methofs ################################
...
rlie_sa_plot <- ggplot(RLIEsa, aes(x = Year, y = RLIE, group = Zone)) +
  ...
  #export the graph
ggsave(
  filename = "outputs/rlie_sa_plot.png", # File name
  plot = rlie_sa_plot,                   # Plot object

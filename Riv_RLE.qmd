...
# average condition estimates for use in D2 rate of change  - this is an adjusted per type leng reflecting condition of the type
riv_scores <- riv %>%
  mutate(cond24 = case_when(PES2024 == "A" ~ 0.9, 
                            PES2024 == "B" ~ 0.65,
                            PES2024 == "C" ~ 0.4,
                            PES2024 == "D" ~ 0.2,
                            PES2024 == "E" ~ 0.08,
                            PES2024 == "F" ~ 0.04,
                            TRUE ~ 0), # DDef not counting towards total score
         cond18 = case_when(PES2018 == "A" ~ 0.9, 
                            PES2018 == "B" ~ 0.65,
                            PES2018 == "C" ~ 0.4,
                            PES2018 == "D" ~ 0.2,
                            PES2018 == "E" ~ 0.08,
                            ...
                            TRUE ~ 0)) %>% # DDef not counting towards total score
    group_by(RIVTYPE) %>%
    summarise(score24 = sum(cond24 * leng, na.rm = TRUE), 
              score18 = sum(cond18 * leng, na.rm = TRUE)) %>%
  ungroup() %>%
  left_join(riv_type_24, by  = "RIVTYPE") %>% 
  mutate(
  condindex24 = pmin(pmax(score24 / (tot_leng24 -`Data defic`), 0), 1), # divide by leng less the DDfec
  condindex18 = pmin(pmax(score18 / (tot_leng24 -`Data defic`), 0), 1)
) %>%
select(RIVTYPE, condindex24, condindex18)
...
pes18_stats <- riv_type_18 %>%
    bind_rows(summarise(., RIVTYPE = "Total", across(where(is.numeric), ~sum(., na.rm = TRUE)) ) ) %>%
  mutate(prp18_ab = (A+B)/tot_leng18)
...
# Join the tables above - this makes a single table in which all the required metric can be found to compute EPL and RLE
 ...
 # Data Defic excluded (segments with PES treated as sample of PES for type) 
  ...
  left_join(eoo_df, by = "RIVTYPE") %>%
     left_join(riv_scores, by = "RIVTYPE") %>%
 # Metrics for RLE 
 # Calculate rate of decline in PES scores over a 50 year period 2018 to 2068    
  mutate(condindex68 = pmin(pmax(condindex24 - ((condindex18 - condindex24) / (2024 - 2018) * 44), 0), 1)) %>%
  mutate(cond_decline = pmin(pmax((condindex18 - condindex68) / condindex18, 0), 1)) %>%
 ...
 #D2b decline in PES  over 50 year period 
    mutate(D2_24 = case_when(    cond_decline >= 0.8 ~ "CR", 
                                 cond_decline >= 0.5 ~ "EN",
                                 cond_decline >= 0.3 ~ "VU", 
                                 TRUE ~ "LC")) %>%
   mutate(D2_18 = D2_24) %>% # PES 2014 unreliable to cannot be used to get rate - assumed to be same
 ...
 # make all assessments factors to get MAX 
    mutate(across( c(B1_18, B1_24,D2_18, D2_24, D3_18, D3_24), 
                   ~ factor(., levels = c("LC", "VU", "EN", "CR"), ordered = TRUE)  ))  %>%
 # Ensure highest risk cateroy is assiged 
   rowwise() %>%
     mutate(RLE_18 = max(c_across(c(B1_18, D2_18, D3_18)), na.rm = TRUE), 
            RLE_24 = max(c_across(c(B1_24, D2_24, D3_24)), na.rm = TRUE)) %>%
   ungroup() %>%
   # add trigger text 
    rowwise() %>%
  mutate(trigger18 = paste(c("B1_18", "D2_18", "D3_18")
                           [c_across(c(B1_18, D2_18,D3_18)) == RLE_18], collapse = ", ")) %>%
  mutate(trigger24 = paste(c("B1_24", "D2_24", "D3_24")
                           [c_across(c(B1_24, D2_24, D3_24)) == RLE_24], collapse = ", ")) %>%
   mutate(trigger18 = ifelse(RLE_18 == "LC", "", trigger18), 
          trigger24 = ifelse(RLE_24 == "LC", "", trigger24)) %>%
   ungroup() %>%
